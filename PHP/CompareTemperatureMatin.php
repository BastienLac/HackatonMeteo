...
$sql2 = $cnx->prepare("select AVG(humidite) from capteur WHERE DAY(date) = DAY(DATE_SUB(DATE(NOW()), INTERVAL 1 DAY)) AND HOUR(date) <= 8");
$sql2->execute();
$moyenneHierHum = $sql2->fetch()[0];


$rqt2 = $cnx->prepare("select AVG(humidite) from capteur WHERE DAY(date) = DAY(DATE(NOW())) AND HOUR(date) <= 8");
$rqt2->execute();
$moyenneTodayHum = $rqt2->fetch()[0];
