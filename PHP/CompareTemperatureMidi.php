...
$sql1 = $cnx->prepare("select AVG(humidite) from capteur WHERE DAY(date) = DAY(DATE_SUB(DATE(NOW()), INTERVAL 1 DAY)) AND HOUR(date) >= 8 AND HOUR(date) <= 16");
$sql1->execute();
$moyenneHierHum = $sql1->fetch()[0];


$rqt1 = $cnx->prepare("select AVG(humidite) from capteur WHERE DAY(date) = DAY(DATE(NOW())) AND HOUR(date) >= 8 AND HOUR(date) <= 16");
$rqt1->execute();
